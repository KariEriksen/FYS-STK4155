...
\begin{itemize}
  \item[(1)] Compute output $\text{NN}(x)=\hat y$ of the artificial neural network, and evaluate the \emph{cost} function, e.g.\ $C(\hat y)\equiv \Vert y-\hat y\Vert_2$. 
...
\begin{align}
C(\hat{\mathbf{y}}) = \Vert \mathbf{y}-\hat{\mathbf{y}}\Vert_2= \sqrt{\sum_{i=1}^{N_\text{O}}(y_i-\hat y_i)^2}. \label{eq:cost}
\end{align}
In general, the output of the neural network is a vector of values, $\mathbf{y}$, and the cost function is taken across all outputs. In \eq{cost}, the network produces $N_\text{O}$ outputs for each input (which itself may be a vector). 
...
\begin{align}
\delta^L_j&=\frac{\partial C(W^L)}{\partial z_j^L} = \frac{\partial C(W^L)}{a_j^L}\frac{\partial a_j^L}{\partial z_j^L}  \nonumber \\
&=
\end{align}
