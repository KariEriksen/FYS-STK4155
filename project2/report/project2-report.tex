...
\begin{align}
 ...
 &= \prod_{i=1}^n \sigma(\beta \cdot \mathbf{x}^{(i)})^{y_i} ( 1-\sigma(\beta \cdot \mathbf{x}^{(i)}))^{1-y_i}
\end{align}
...
\begin{align}
 l(\beta) &= \log(L(\beta)) \nonumber \\
 &= \sum_{i=1}^n \Big[y_i \sigma(\beta \cdot \mathbf{x}^{(i)}) \nonumber \\
 & \ \ \ \  \ \ \ \ + (1-y_i)( 1-\sigma(\beta \cdot \mathbf{x}^{(i)}) )\Big]. \nonumber
\end{align}
...
\begin{equation}
 C(\beta) \equiv  -l(\beta) \label{logRegCrossEntropy}.
\end{equation}
...
\begin{equation}
 \frac{\partial}{\partial \beta_j} C(\beta) = - \sum_{i=1}^n \mathbf{x}_j^{(i)} \left( y^{(i)} - \sigma(\beta \cdot \mathbf{x}^{(i)}) \right)
\end{equation}
...
\mathbf{y} &\equiv (y_1,\dots,y_n), \\       
      \mathbf{p} &\equiv \left( \sigma(\beta \cdot \mathbf{x}^{(1)}),\dots,\sigma(\beta \cdot \mathbf{x}^{(n)}) \right)
      \end{align}
...
We show here that the cross-entropy (\ref{logRegCrossEntropy}) is a convex function. In the following we 
write $p_i \equiv \sigma(\beta \cdot \mathbf{x}^{(i)})$. 
...
&= \sym J + D.
\end{align}
As we can see, we are tantalizingly close to recreating the symmetric part of $J$\textemdash the only difference is the diagonal elements, which we may organize in a matrix $D$ with elements\footnote{The $D_i$ and the $D_i'$ elements shown in $(W+W^T)/2$ are related by $D_i=-D_i'/2$, where we simply absorbed the constant into $D_i'$ for ease of notation.} $D_i$. It is obvious that since the original $\mathbf{s}^TW\mathbf{s}=\mathbf{s}^TJ\mathbf{s}$, and the quadratic form is invariant under any $A\mapsto \sym A$ transformation, then $\mathbf{s}^T\sym W\mathbf{s}=\mathbf{s}^T(\sym J + D)\mathbf{s}$ which can only be true if $\mathbf{s}^TD\mathbf{s}=0$. In general, quadratic forms are not invariant under $A\mapsto A+D$, with $D$ a diagonal matrix. However, in our special case\textemdash recall that the input state can only take values $s_i=\pm 1$\textemdash it is, if and only if $\text{Tr}\,D=0$. 
...
This outlines a fundamental difficulty in working with machine learning algorithms such as the present one; it is essentially a \textit{black box}. Even when it works perfectly, it is not always easy to pinpoint \textit{why} or \textit{how} it works.
